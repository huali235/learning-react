...
function reducer(state, action) {
  switch (action.type) {
    case "open":
      return {
        ...state,
        balance: state.balance + OPENING_AMOUNT,
        isActive: true,
      };
    case "deposit":
      return {
    ...
    case "withdraw":
      return {
        ...state,
        balance:
          state.balance === 0 ? state.balance : state.balance - WITHDRAW_AMOUNT,
        isActive: state.isActive ? true : false,
      };
    case "request":
      return {
        ...state,
        loan: state.loan > 0 ? state.loan : state.loan + LOAN_AMOUNT,
    ...
    case "close":
      if (state.loan === 0 && state.balance === 0) {
        ...
        return { ...state, isActive: state.isActive ? true : false };
      }
    default:
